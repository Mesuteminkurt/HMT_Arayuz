...
int main(void)
{
  ...
  // Modemin RST pinini normal (çalışma) konumuna al
  HAL_GPIO_WritePin(RST_GPIO_Port, RST_Pin, GPIO_PIN_SET);

  // Modemi aktif et (VCC Enable mantığı: 1 yapıp bırakıyoruz)
  HAL_GPIO_WritePin(GPIOA, PWR_Pin, 1);
  
  // Soğuk açılışlarda modemin tamamen boot etmesi için uzun bir bekleme süresi şart.
  // 300ms veya 1000ms modemin uyanması için yeterli değildir.
  HAL_Delay(6000);
  ...
  HAL_Delay(2000);
  Cavli_Send("AT+MQTTCONN=3,0");                   HAL_Delay(5000);
